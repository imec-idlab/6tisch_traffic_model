...
SF_PERIOD       = 3.97      # Default 6TiSCH Minimal schedule contiki-NG
...
IMAX            = 1048.0
L_DIO           = 96        # DIO length
DAO_PERIOD      = 15.0*60   # DAG lifetime, between 15 min and 22.5 min (RPL classic)
L_DAO           = 76        # DAO length
L_DAO_ACK       = 34        # DAO ACK length
...
def get_multihop_coap_bytes(path, payload, mac_hdr, tot_hdr, n_coap):
    ...
    mh_first = get_fragmenting(payload, mac_hdr, tot_hdr, 8)
    mh_inter = get_fragmenting(payload, mac_hdr, tot_hdr, 17)
    mh_last = get_fragmenting(payload, mac_hdr, tot_hdr, 9)
    etx = 1
    if(len(path) == 2):
        ...
        path[1].eacktx += len(mh_first)*L_EACK*n_coap
        path[1].coaprx += sum(mh_first)*n_coap
        path[0].eackrx += len(mh_first)*L_EACK*n_coap
        for i in range (1, len(path)-2):
            etx = ns.get_etx(path[i], path[i+1])
            path[i].coaptx += sum(mh_inter)*n_coap*etx
            path[i+1].eacktx += len(mh_inter)*L_EACK*n_coap
            path[i+1].coaprx += sum(mh_inter)*n_coap
            path[i].eackrx += len(mh_inter)*L_EACK*n_coap
        etx = ns.get_etx(path[-1], path[-2])
        path[-2].coaptx += sum(mh_last)*n_coap*etx
        path[-1].eacktx += len(mh_last)*L_EACK*n_coap
        path[-1].coaprx += sum(mh_last)*n_coap
        path[-2].eackrx += len(mh_last)*L_EACK*n_coap
...
def get_p2p_coap_bytes(nodes, rq_payload, rp_payload, mac_hdr, tot_hdr, p_time):
    ...
    for n in nodes:
        if(n.id != SERVER):
            ...
            request_path = get_multihop_path(n, ns.search_node(SERVER))
            response_path = get_multihop_path(ns.search_node(SERVER), n)
...
def get_multihop_dao_bytes(path, n_dao, l_dao, dao):
    if(len(path) == 2):
        ...
        path[-1].eacktx += L_EACK*n_dao
        path[0].eackrx += L_EACK*n_dao
    elif(len(path) > 2):
        etx = ns.get_etx(path[0], path[1])
        if(dao):
            path[0].daotx += l_dao*n_dao*etx
        ...
        for i in range (1, len(path)-2):
            ...
            path[i+1].eacktx += L_EACK*n_dao            
            path[i].eackrx += L_EACK*n_dao
        etx = ns.get_etx(path[-1], path[-2])
        if(dao):
...
def get_daos(nodes, p_time, opt_tel):
    ...
    for n in nodes:
        ...
        if(n.id != ROOT):
            ...
            l_dao = L_DAO + 23 + n.n + len(n.rt)*2
            path = get_multihop_path(n,ns.search_node(ROOT))
            get_multihop_dao_bytes(path,n_dao,l_dao,1)
            get_multihop_dao_bytes(path[::-1],n_dao,L_DAO_ACK,0)
    ...
    with open('predictions.csv', 'w', newline='') as csvfile:
        ...
        while(ns.cont):
            ...
            if(len(ns.nodes) == ns.N):
                ...
                for n in ns.nodes:
                    if(realtime):
                        n.print_n(1, 1, prediction_interval)
